...
class PPTXExtractionCollection(ActionCollection):
    ...
    def __init__(self, arguments: ActionArguments) -> None:
        ...
        print("PPTX Extraction Service initialized")
        print(f"Media output directory: {self._media_output_dir}")

    def _validate_file_path(self, file_path: str) -> Path:
        """Validate and convert file path string to Path object"""
        path = Path(os.path.expanduser(file_path))
        if not path.exists():
            raise FileNotFoundError(f"File not found: {file_path}")
        if path.suffix.lower() not in self.supported_extensions:
            raise ValueError(f"Unsupported file type: {path.suffix}. Supported types: {self.supported_extensions}")
        return path
            ...
            with zipfile.ZipFile(file_path, "r") as zip_file:
                ...
                for idx, media_file in enumerate(media_files):
                    try:
                        ...
                        print(f"Failed to extract media {media_file}: {e}")

        except Exception as e:
            print(f"Failed to extract media from PPTX: {e}")
    ...
    def _extract_content_from_pptx(self, file_path: Path, extract_notes: bool = True) -> dict[str, Any]:
        ...
        try:
            ...
            for slide_idx, slide in enumerate(presentation.slides):
                ...
                if extract_notes and hasattr(slide, "notes_slide"):
                    try:
                        ...
                        print(f"Failed to extract notes from slide {slide_idx + 1}: {e}")
            ...
            print(f"Failed to extract content from PPTX: {e}")
            raise
    ...
    try:
        ...
        if is_mcp_mode:
            ...
            try:
                ...
                function_name = input_data.get("function_name", input_data.get("name", "extract_pptx_content"))
                arguments = input_data.get("arguments", {})
                
                if function_name == "mcp_extract_pptx_content":
                    result = service.mcp_extract_pptx_content(
                        file_path=arguments.get("file_path", ""),
                        output_format=arguments.get("output_format", "markdown"),
                        extract_images=arguments.get("extract_images", True),
                        extract_notes=arguments.get("extract_notes", True),
