...
class ImageCollection(ActionCollection):
    ...
    def mcp_extract_text_ocr(
        ...
        try:
            ...
            print(f"Performing OCR on: {file_path.name}")
            ...
            print(f"OCR completed: {word_count} words extracted in {processing_time:.2f}s")
    ...
    def mcp_analyze_image_ai(
        ...
        try:
            ...
            # Validate input file
            file_path: Path = self._validate_file_path(file_path)
            print(f"Analyzing image with AI: {file_path.name}")
            ...
            print(f"AI analysis completed in {processing_time:.2f}s")
    ...
    def mcp_list_supported_formats(self) -> ActionResponse:
        """List all supported image formats for processing.

        Returns:
            ActionResponse with list of supported image formats and their descriptions
        """
        supported_formats = {
            "JPEG": "Joint Photographic Experts Group (.jpg, .jpeg) - Common lossy compression format",
            "PNG": "Portable Network Graphics (.png) - Lossless compression with transparency support",
            "GIF": "Graphics Interchange Format (.gif) - Supports animation and transparency",
            "WEBP": "WebP (.webp) - Modern format with both lossy and lossless compression",
            "BMP": "Bitmap (.bmp) - Uncompressed raster image format",
            "TIFF": "Tagged Image File Format (.tiff, .tif) - High-quality format for print",
            "ICO": "Icon (.ico) - Used for website favicons and application icons",
            "SVG": "Scalable Vector Graphics (.svg) - Vector-based format for web graphics",
        }
