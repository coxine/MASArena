...
class ChessCollection(ActionCollection):
    ...
    def __init__(self, arguments: ActionArguments) -> None:
        ...
        print("Chess service initialized")
        print(f"Initial board FEN: {self.board.fen()}")
    ...
    async def mcp_make_move(
        ...
        try:
            ...
            print(f"♟️ Move made: {move_str} (UCI: {move.uci()}, SAN: {move_san})")
    ...
    async def mcp_get_legal_moves(
        ...
        if output_format == "uci_list":
            ...
            try:
                ...
                print(f"⚠️ Could not generate all SAN moves: {e}. Falling back to UCI for problematic moves.")
                san_moves = []
                for move in self.board.legal_moves:
                    try:
