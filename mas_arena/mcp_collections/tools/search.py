...
from mas_arena.mcp_collections.base import ActionArguments, ActionCollection, ActionResponse
...
class SearchCollection(ActionCollection):
    ...
    def __init__(self, arguments: ActionArguments) -> None:
        ...
        # Check if we're running in MCP tool mode
        is_mcp_mode = len(sys.argv) == 1
        
        if is_mcp_mode:
            # Write to stderr instead of stdout
            sys.stderr.write("Search service initialized\n")
            
            if self.google_api_key and self.google_cse_id:
                sys.stderr.write("Google Search API credentials found\n")
            else:
                sys.stderr.write("Google Search API credentials missing - some features may be unavailable\n")
        else:
            # Normal output
            ...
            if self.google_api_key and self.google_cse_id:
                ...
                print("Google Search API credentials missing - some features may be unavailable")
    ...
    def mcp_search_google(
        ...
        try:
            ...
            # Log search query to stderr if in MCP mode
            if len(sys.argv) == 1:
                sys.stderr.write(f"🔍 Searching Google for: '{validated_query}'\n")
            else:
                print(f"🔍 Searching Google for: '{validated_query}'")
            ...
            # Log results to stderr if in MCP mode
            if len(sys.argv) == 1:
                sys.stderr.write(f"✅ Found {len(search_results)} results in {search_time:.2f}s\n")
            else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        ...
        # Test parameters
        test_query = "artificial intelligence latest research"
        test_num_results = 5
        ...
        result = service.mcp_search_google(test_query, test_num_results, True, "en", "us", test_format)
    ...
    try:
        ...
        if len(sys.argv) == 1:
            ...
            try:
                ...
                tool_name = input_data.get("name", "search")
                arguments = input_data.get("arguments", {})
                
                # Call the appropriate tool
                if tool_name == "mcp_search_google":
                    result = service.mcp_search_google(
                    ...
                    sys.stdout.flush()
                    sys.exit(0)
                if tool_name == "mcp_get_search_capabilities":
                    result = service.mcp_get_search_capabilities()
                    sys.stdout.write(json.dumps(result.model_dump()) + "\n")
                    sys.stdout.flush()
                    sys.exit(0)
                else:
                    error = {"success": False, "message": f"Unknown tool: {tool_name}"}
